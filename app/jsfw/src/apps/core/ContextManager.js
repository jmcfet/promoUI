	...
	function (Log, Context, ContextManagerHelper, WatchDog) {
		...
		$N.apps.core.ContextManager = (function () {
			...
			var switchContext = function (fromCtx, toCtx, activationContext, navCompleteCallback) {
				...
				if (fromCtx) {
					while (underlayContexts.length > 0) {
						hideContext(underlayContexts.pop());
					}
			...
			return {
				 ...
				 *							Either `DIV` or `null`
				 */
				addContext : function (id, url, cacheStrategy, isDefault, x, y, height, width, group, htmlType) {
				...
				overlay : function (cUrlOrName, activationContext, navCompleteCallback, hideUnderlays) {
					...
					if (hideUnderlays) {
						...
						for (i = 0, end = underlayContexts.length; i < end; i++) {
							hideContext(underlayContexts[i], true);
						}
					}
				...
				popOverlay : function (callbackData, leaveAsPreview) {
					...
					if (underlayContexts.length > 0) {
						if (leaveAsPreview) {
							...
							hideContext(activeContext);
						}

						// show underlying contexts
						for (i = underlayContexts.length - 1; i >= 0; i--) {
							if (!underlayContexts[i].getHiddenBy() ||  underlayContexts[i].getHiddenBy() === activeContext) {
								domAbstraction.showContextElement(underlayContexts[i].getHandle());
							} else if (underlayContexts[i].getHiddenBy() &&  underlayContexts[i].getHiddenBy() !== activeContext) {
								break;
							}
						}

						activeContext = underlayContexts.pop();
						var currentUnderlay = underlayContexts[underlayContexts.length - 1] || null;
						currLoadingId = currentUnderlay ? currentUnderlay._id : null;
					...
					if (cUrl) {
						if (status === READY) {
							...
							if (asOverlay) {
								...
								underlayContexts.push(activeContext);
				...
				navigateToDefaultContext : function (activationContext, navCompleteCallback) {
					var defaultNavigationLink = this.getNavigationLink(this.getDefaultContext().getId());
