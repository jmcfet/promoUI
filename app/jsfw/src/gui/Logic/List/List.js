...
define('jsfw/gui/Logic/List/List',
    [],
    function () {
		$N = window.$N || {};
		$N.gui = $N.gui || {};
		 ...
		 * @param {Object} rowObject
		 */
		$N.gui.CyclicPagingList = function (rowObject) {
			var my = {},
				superClass = {};
			...
			this.selectRowAtIndex = function (index) {
				var newFirstIndex,
					newLastIndex;
				if (this.getWrapAround() && (my.lastRow > this.getVisibleRows())) {
					newFirstIndex = index % my.lastRow;
					newLastIndex = (index + this.getVisibleRows() - 1) % my.lastRow;
					...
					superClass.selectRowAtIndex.call(this, index);
				}
			};
		...
		$N.gui.StandardList = function (rowObject, my) {
			...
			// TODO: this need beefing up to allow for scrolls.
			/**
			 ...
			 * @return {Number}
			 */
			this.getVisibleRows = function () { //TODO: all references to rows should be items
				return visibleRows;
			};
		...
		$N.gui.BasicList = function (rowObject, my) {
			 ...
			 */
			this.addItem = function (arg1, arg2, arg3, arg4) {
				//if (my.lastRow < this.getMaxItems()) { //TODO: fix this
				/*jslint newcap:true*/
			...
			/**
			 * Adds the given data to the beginning of an existing list
			 * @method prependData
			 * @param {Array} data
			 */
			this.prependData = function (data) {
				my.listItems = data.concat(my.listItems);
				my.lastRow = my.listItems.length;
			};
