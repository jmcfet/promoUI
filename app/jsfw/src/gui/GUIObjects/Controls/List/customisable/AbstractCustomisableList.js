 ...
 * @requires $N.gui.PresentationManager
 * @requires $N.Config
 *
 * @constructor
 * @param {Object} docRef Document reference.
 */
define('jsfw/gui/GUIObjects/Controls/List/customisable/AbstractCustomisableList',
    [
    ...
    function (List, Util, GUIObject, FrameworkCore, PresentationManager, Config) {
		...
		/**
		 * Adds data to the front of the current list.
		 * @method prependData
		 * @param {Object} data The data to be prepended to the beginning of the list.
		 */
		AbstractCustomisableList.prototype.prependData = function (data) {
			this._data.prependData(data);
		};
		...
		/**
		 * Sets the behaviour to perform immediately (regardless of _itemHighlightedTimeoutMS )
		 ...
		 * @method select
		 * @param {Boolean}  optional flag to invoke highlight callback immediately
		 */
		AbstractCustomisableList.prototype.select = function (immediate) {
			...
			if (immediate) {
				...
				this._handleItemHighlighted(selectedItem);
			}
		 ...
		 */
		AbstractCustomisableList.prototype.selectPrevious = function () {
		 ...
		 * false if movement to the next list item was not made.
		 */
		AbstractCustomisableList.prototype.selectNext = function () {
		 ...
		 */
		AbstractCustomisableList.prototype._handleItemHighlighted = function (item) {
			var me = this;
			this.cancelSelect();
			if (this._itemHighlightedImmediateCallback) {
				this._itemHighlightedImmediateCallback(item);
			}
			this._itemHighlightedTimeout = setTimeout(function () { me._itemHighlightedCallback(item); }, this._itemHighlightedTimeoutMS);
		};
		 ...
		 */
		AbstractCustomisableList.prototype.setOrientation = function (orientation) {
		...
		*/
		 ...
		 */
		AbstractCustomisableList.prototype.keyHandler = function (key) {
			var keys = $N.gui.FrameworkCore.getKeys();
			switch (key) {
		...
		return AbstractCustomisableList;
	}
);