/**
 * This class retrieves EPG data directly from SDP when requested; it doesn't cache it.
 *
		...
		$N.services.sdp.EPG = (function () {
			...
			return {
				...
				init: function (account, currentLocale) {
					...
					btvBrowsingService = $N.services.sdp.ServiceFactory.get('BtvBrowsingService');
				...
				fetchAllChannels: function (successCallback, failureCallback) {
					...
					$N.services.sdp.ServiceFactory.get("ContextService").getCurrentContext(this, function (result) {
						var request = {};
						//TODO: A cutdown context is being used because of a SDP bug where a deviceUid of 0 was returned
						//after signOnByUser. This may limit the channels being returned.
						//request.context = result;
						request.context = {};
						request.context.spid = result.spid;
						request.context.locale = result.locale;
						request.filter = {};
						request.sortOrder = ['callLetters'];
						btvBrowsingService.getChannels(this, function (results) {
							var i,
								j,
								technicalItem,
								channels = [];
							...
							for (i = 0; i < results.length; i++) {
								for (j = 0; j < results[i].channelResponseTechnicalItems.length; j++) {
									technicalItem = results[i].channelResponseTechnicalItems[j].technicalChannel;
									if (technicalItem.networkLocation) {
										channels.push(technicalItem);
									}
								}
							}
							executeChannelsCallback(channels, successCallback);
						}, failureCallback, JSON.stringify(request));
					}, failureCallback);
