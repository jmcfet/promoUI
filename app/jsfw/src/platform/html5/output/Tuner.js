...
define('jsfw/platform/output/Tuner',
	...
	function (Log) {
		...
		proto._tuneWithPlayOption = function (uri, playOption) {
			...
			if (this.getCurrentUri() !== uri) {
				if (this._ready) {
					...
					this._ready = false;
					this._player.setAttribute("src", uri);
					this._player.load();
					if (playOption) {
						this._player.play();
					}
		...
		proto.getCurrentUri = function () {
			try {
				return this._player.getAttribute("src");
			} catch (e) {
				return null;
			}
		};
		...
		proto.disconnect =  function () {
			...
			if (this._player) {
				this._player.setAttribute("src", null); //may need browser abstraction
			}
			log("disconnect", "EXIT");
		};
		...
		proto.showFullscreen = function (videoElement) {
			...
			if (videoElement) {
				...
				log("showScaled", "Restoring video window to full screen");
			} else {
				log("showScaled", "Failed to restore video window no element found");
			}
