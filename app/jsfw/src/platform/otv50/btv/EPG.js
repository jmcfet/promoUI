...
define('jsfw/platform/btv/EPG',
	[
		...
		'jsfw/services/gateway/HomeCruise',
		'jsfw/platform/btv/EPGCache'
	],
	function (Log, EPGService, EPGServiceFactory, EPGEventFactory, PersistentCache, EPG, HomeCruise, EPGCache) {
		window.$N = $N || {};
		...
		$N.platform.btv.EPG = (function () {
			...
			return {
				...
				init: function (configuration) {
					...
					if (configuration.extendedIPSchedule) {
						isExtendedIPSchedule = true;
					}
					...
					if (configuration.dataSources && configuration.dataSources.length > 0) {
						for (i = 0; i < configuration.dataSources.length; i++) {
							...
							if (currentDataSource === DATA_SOURCES.GATEWAY && $N.services.gateway.HomeCruise) {
				...
				getEventsByWindow: function (serviceIds, startTime, endTime) {
					log("getEventsByWindow", "begin with startTime:" + startTime + " endTime:" + endTime, Log.LOG_DEBUG);
				...
				getEventById: function (eventId) {
					log("getEventById", "for event " + eventId, Log.LOG_DEBUG);
				...
				getCurrentEventForService: function (serviceId) {
					log("getCurrentEventForService", "for service " + serviceId, Log.LOG_DEBUG);
				...
				getNextEventForService: function (serviceId) {
					log("getNextEventForService", "for service " + serviceId, Log.LOG_DEBUG);
				...
				getPreviousEventForService: function (serviceId) {
					log("getPreviousEventForService", "for service " + serviceId, Log.LOG_DEBUG);
					var event = this.getCurrentEventForService(serviceId),
						eventId = event ? event.eventId : null;
				...
				getEventsForGridRow: function (serviceId, startTime, endTime, getDummyEvent) {
					log("getEventsForGridRow", "begin with service " + serviceId, Log.LOG_DEBUG);
				...
				fetchCurrentEventForService: function (serviceId, callback) {
					log("fetchCurrentEventForService", "for service " + serviceId, Log.LOG_DEBUG);
					var startTime,
						service,
				...
				fetchNextEventForService: function (serviceId, callback) {
					log("fetchNextEventForService", "for service " + serviceId, Log.LOG_DEBUG);
					var startTime,
						service,
						me = this,
						criteria = "",
				...
				fetchPreviousEventForService: function (serviceId, callback) {
					log("fetchPreviousEventForService", "for service " + serviceId, Log.LOG_DEBUG);
					var startTime,
						service,
						me = this,
						results,
				...
				fetchEventsByWindow: function (serviceIds, startTime, endTime, callback) {
					log("fetchEventsByWindow", "for services " + serviceIds.length, Log.LOG_DEBUG);
					var newEvents = [],
						fetchedEvents = null,
						serviceGaps = {},
						services = {},
				...
				fetchEventsForChannelWithDummyEvents: function (serviceId, startTime, endTime, getDummyEvent, callback) {
					log("fetchEventsForChannelWithDummyEvents", "for service " + serviceId, Log_DEBUG);
