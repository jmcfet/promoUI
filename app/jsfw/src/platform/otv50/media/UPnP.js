		...
		$N.platform.media.UPnP = (function () {
			...
			function browseComplete(e) {
				if (getContentsCallback) {
					getContentsCallback(processContentResults(e.content));
				}
			}
			...
			return {
				...
				getRootContentsForDevice: function (device, callback) {
					...
					CCOM.HomeNetworking.browseContainer(device.udn, "0", "", "*", 0, MAX_CONTENT_ITEMS);
				},
				...
				getContentsForFolder: function (folder, callback, start, limit) {
					...
					CCOM.HomeNetworking.browseContainer(currentDevice.udn, folder.id, "", "*", start, limit);
				},
