	...
	function (Log, Tuner, TrickPlay, SeekTrickPlay, Tracks, Util) {
		...
		function VideoPlayer(videoTag) {
			...
			if (videoTag) {
				...
				paramElement.setAttribute("name", "otv-video-destination");
				paramElement.setAttribute("value", "display://" + instanceCount);
				videoTag.appendChild(paramElement);
			}
			...
			var playerManager = CCOM.PlayerManager,
				priority = instanceCount === 0 ? playerManager.INSTANCE_PRIORITY_HIGH : playerManager.INSTANCE_PRIORITY_MEDIUM,
			...
			if (instanceResult.instance) {
				...
				playerManager.setReviewBufferState(this._player, enableReviewBuffer);
				playerManager.setAudioStatus(this._player, audioStatus);
				this._player.videoLayerProperties = {zorder: instanceCount === 0 ? this._player.VIDEO_LAYER_ZORDER_BOTTOM : this._player.VIDEO_LAYER_ZORDER_TOP};
			} else {
				this._player = null;
