...
define('jsfw/gui/GUIObjects/Animations/AnimateHTML',
    ...
    function () {
		...
		function AnimateHTML(guiObject, attribute, timingFunction) {
		...
		AnimateHTML.prototype._animatedCSSMove = function (fromX, fromY, toX, toY, is3DAnimation) {
			...
			// NETUI-2928: Non-linear animation support
			this._guiObject.getRootElement().style["-webkit-transition-timing-function"] = null;
			this._guiObject.getRootElement().style["transition-timing-function"] = null;
			...
			AnimateHTML._animationQ["" + this._animID] = function () {
				...
				// NETUI-2928: Non-linear animation support
				if (me._timingFunction) {
				me._guiObject.getRootElement().style["-webkit-transition-timing-function"] = me._timingFunction;
				me._guiObject.getRootElement().style["transition-timing-function"] = me._timingFunction;
				}
		...
		/**
		 * Performs an animated rotate on the gui object
		 * @method animatedRotate
		 * @param {Number} from
		 * @param {Number} to
		 * @param {Boolean} repeat whether the animation should repeat
		 * @param {Boolean} bounce whether the animation should reverse
		...
		AnimateHTML.prototype.animatedRotate = function (from, to, repeat, bounce) {
			var me = this;
			this._animation = function (toValue) {
				me._updateWebkitTransform("rotate", toValue + "deg");
			};
			this._setupRepeatAnimation(from, to, repeat, bounce);
			this._doAnimation(from, to);
		};
