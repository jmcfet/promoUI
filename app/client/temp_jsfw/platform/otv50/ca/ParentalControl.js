...
// This file has been added to support NET ticket NETUI-1125. This file should be removed
// once the JSFW has been fixed NINJA-1711.
...
$N.platform.ca.ParentalControl = (function () {
	var log = new $N.apps.core.Log("ca", "ParentalControl"),
		...
		setAllRestrictedChannelsListener = function () {},
		addRestrictedChanneListener = function () {},
		removeRestrictedChannelListener = function () {},

		removeAllRestrictedChannelsListener = function () {},
		ratingLookUp = {
			'0': {
				ratingCode: "U",
	...
	function cacheRestrictedAndTimeLockedServices(restrictedChannelsToCache) {
		log("cacheRestrictedAndTimeLockedServices", "Enter");
		var i,
			j,
			ageWindowStartIndex,
			timeWindowObj,
		...
		for (i = 0; i < restrictedChannelsToCache.length; i++) {
			log("cacheRestrictedAndTimeLockedServices", "Processing: " + restrictedChannelsToCache[i]);
			ageWindowStartIndex = restrictedChannelsToCache[i].indexOf("[/age:");
			if (ageWindowStartIndex !== -1) {
				serviceId = restrictedChannelsToCache[i].substring(0, ageWindowStartIndex);
				log("cacheRestrictedAndTimeLockedServices", "Caching restricted status for " + serviceId);
				restrictedChannels[serviceId] = true;
			} else {
				...
				if (timeWindowStartIndex !== -1) {
					...
					log("cacheRestrictedAndTimeLockedServices", "Caching restricted status for " + restrictedChannelsToCache[i]);
					restrictedChannels[restrictedChannelsToCache[i]] = true;
				}
			}
		}
		log("cacheRestrictedAndTimeLockedServices", "Exit");
	}
	...
	function enableSystemListener(e) {
		if (!e.error) {
			...
			ccomUserAuth.getAllRestrictedChannels();
		}
	}
	 ...
	 * @method getAllRestrictedChannelsListener
	 * @param {Object} e
	 */
	function getAllRestrictedChannelsListener(e) {
		log("getAllRestrictedChannelsListener", "Enter");
		if (e.channelList) {
			log("getAllRestrictedChannelsListener", "channelList: " + e.channelList);
			cacheRestrictedAndTimeLockedServices(e.channelList);
		} else if (e.error && e.error.name) {
			log("getAllRestrictedChannelsListener", "erroe: " + e.error.name);
		}
	...
	return {
		...
		initialise: function (ratingLookup) {
			...
			ccomUserAuth.addEventListener("addRestrictedChannelOK", addRestrictedChanneListener);
			ccomUserAuth.addEventListener("addRestrictedChannelFailed", addRestrictedChanneListener);
			ccomUserAuth.addEventListener("getAllRestrictedChannelsOK", getAllRestrictedChannelsListener);
			ccomUserAuth.addEventListener("getAllRestrictedChannelsFailed", getAllRestrictedChannelsListener);
			ccomUserAuth.addEventListener("setAllRestrictedChannelsOK", setAllRestrictedChannelsListener);
			ccomUserAuth.addEventListener("setAllRestrictedChannelsFailed", setAllRestrictedChannelsListener);
			ccomUserAuth.addEventListener("removeRestrictedChannelOK", removeRestrictedChannelListener);
			ccomUserAuth.addEventListener("removeRestrictedChannelFailed", removeRestrictedChannelListener);
		...
		isServiceAndEventPermitted: function (epgService, epgEvent) {
			...
			if (isCurrentUserMaster || !epgService || !epgEvent) {
				log("isServiceAndEventPermitted", "exit 1 isCurrentUserMaster " + isCurrentUserMaster);
				return true;
			}

			if (!this.isRatingPermitted(epgService.parentalRating)) {
				log("isServiceAndEventPermitted", "Exit 2 ");
				return false;
			}

			if (!this.isRatingPermitted(epgEvent.parentalRating)) {
				log("isServiceAndEventPermitted", "Exit 3 ");
				return false;
			}

			if (this.isServiceIdRestricted(epgService.serviceId)) {
				log("isServiceAndEventPermitted", "Exit 4");
				return false;
			}
			...
			if (this.isServiceLockedAtTime(epgService.serviceId, startTime, endTime)) {
				log("isServiceAndEventPermitted", "Exit 5");
				return false;
			}
			log("isServiceAndEventPermitted", "Exit 6");
			return true;
		},
		...
		isServicePermitted: function (epgService) {
			log("isServicePermitted", "Enter & Exit");
			return (!this.isServiceIdRestricted(epgService.serviceId));
		},
		 ...
		 * @return {Boolean}
		 */
		isUserAdult: function () {
			return (isCurrentUserMaster || userRating >= ADULT_AGE_RATING);
		},
		...
		isServiceLockedAtTime: function (serviceId, startTime, endTime) {
			return false; // NET does not use time locks, therefore we're commenting this out as the cost of processing this is non-trivial in ListGuide
		},
		...
		getRestrictedServicesCount: function () {
			...
			return restrictedChannels.length;
		},
		...
		setRestrictedService: function (serviceId, userPin, callback) {
			...
			var i = 0,
				index = -1;
			if (!restrictedChannels.hasOwnProperty(serviceId)) {
				if (!callback) {
					addRestrictedChanneListener = function (e) {
						if (!e.error) {
							callback(true);
							log("addRestrictedChanneListener", "sucess");
						} else {
							callback(false);
							log("addRestrictedChanneListener", "fail");
						}
						addRestrictedChanneListener = function (e) {};
					};
				}
				restrictedChannels[serviceId] = true;
				restrictedChannels.length = restrictedChannels.length + 1;
				if (serviceId) {
					ccomUserAuth.addRestrictedChannel($N.platform.ca.PINHandler.getLocalMasterPin(), serviceId);
				}
			} else {
				if (!callback) {
					callback(true);
		...
		unsetRestrictedService: function (serviceId, userPin, callback) {
			...
			for (i = 0; i < restrictedChannels.length; i++) {
				if (restrictedChannels[i] === serviceId) {
					index = i;
					break;
				}
			}
			if (!restrictedChannels.hasOwnProperty(serviceId) || restrictedChannels[serviceId] === false) {
				if (!callback) {
					callback(true);
				}
			} else {
				if (!callback) {
					removeRestrictedChannelListener = function (e) {
						if (!e.error) {
							callback(true);
							log("removeRestrictedChannelListener", "sucess");
						} else {
							callback(false);
							log("removeRestrictedChannelListener", "fail");
						}
						removeRestrictedChannelListener = function (e) {};
					};
				}
				restrictedChannels[serviceId] = false;
				restrictedChannels.length = restrictedChannels.length - 1;
				if (serviceId) {
					ccomUserAuth.removeRestrictedChannel($N.platform.ca.PINHandler.getLocalMasterPin(), serviceId);
				}
			}
		...
		setRestrictedServices: function (serviceIds, userPin, callback) {
			...
			var i,
				storedRestrictedChannels = new Array();
			if (serviceIds.length > 0) {
				restrictedChannels = {};
				for (i = 0; i < serviceIds.length; i++) {
					restrictedChannels[serviceIds[i]] = true;
					storedRestrictedChannels.push(serviceIds[i]);
				}
				if (callback) {
					setAllRestrictedChannelsListener = function (e) {
						if (!e.error) {
							callback(true);
							log("setAllRestrictedChannelsListener", "sucess");
						} else {
							callback(false);
							log("setAllRestrictedChannelsListener", "fail");
						}
						setAllRestrictedChannelsListener = function (e) {};
					};
					ccomUserAuth.addEventListener("setAllRestrictedChannelsOK", setAllRestrictedChannelsListener);//registering the listeners again as empty initialisation functions were getting called as they already stored with CCOM as listeners
					ccomUserAuth.addEventListener("setAllRestrictedChannelsFailed", setAllRestrictedChannelsListener);
				}
				ccomUserAuth.setAllRestrictedChannels($N.platform.ca.PINHandler.getLocalMasterPin(), storedRestrictedChannels);
			} else {
				unsetAllRestrictedServices(callback);
			}
			log("setRestrictedServices", "Exit");
		},

		/**
		 * Removes all services from the restricted list for the given user
		 * @method unsetAllRestrictedServices
		 * @param {String} [userPin=''] pin number of user to update. If undefined / null then default user is modified
		 * @param {Function} [callback=null] Called once the services restrictions have been unset. If call fails will pass a false
		 * parameter to the callback, if succeeds will pass true parameter
		 */
		unsetAllRestrictedServices: function (callback) {
			log("unsetAllRestrictedServices", "Enter");
			restrictedChannels = {};
			if (callback) {
				removeAllRestrictedChannelsListener = function (e) {
					if (!e.error) {
						callback(true);
						log("removeAllRestrictedChannelsListener", "sucess");
					} else {
						callback(false);
						log("removeAllRestrictedChannelsListener", "fail");
					}
					removeAllRestrictedChannelsListener = function (e) {};
				};
				ccomUserAuth.addEventListener("removeAllRestrictedChannelsOK", removeAllRestrictedChannelsListener);
				ccomUserAuth.addEventListener("removeAllRestrictedChannelsFailed", removeAllRestrictedChannelsListener);
			}
			ccomUserAuth.removeAllRestrictedChannels($N.platform.ca.PINHandler.getLocalMasterPin());
		...
		unsetRestrictedServices: function (serviceIds, userPin, callback) {
			...
			var usersRestrictedChannels = [],
				i,
				serviceIdsIndex = {},
				newRestrictedChannels = new Array();
			restrictedChannels = {};
			if (!userPin) {
				userPin = ""; //default user
			}
		...
		getRestrictedServices: function () {
			...
			log("getRestrictedServices", JSON.stringify(restrictedChannels));
		...
		isServiceIdRestricted: function (serviceId) {
			...
			if (isCurrentUserMaster || !serviceId || !restrictedChannels) {
				log("isServiceIdRestricted", "Exit 1");
				return false;
			}
			if (restrictedChannels.hasOwnProperty(serviceId)) {
				return restrictedChannels[serviceId];
			}
			log("isServiceIdRestricted", "Exit 2");
			return false;
		},
