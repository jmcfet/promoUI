	...
	function (Log, XssRequest, JSON, allServicesPatch) {
		...
		$N.services.sdp.MetadataService = (function () {
			var securityRequired = false,
				...
				VOD_TRAILERS_URI = 'vod/promotions',
			...
			return {
				 ...
				 */
				init: function (serverAddress, port, baseURI, serviceProviderId, timeoutCheckMS, locale, region, deviceType, searchURI, useHttps, publishFlagDefaultStatus, forceInitialise) {
					log('init', 'Enter');
					if (!isInitialised || forceInitialise === true) {
				 ...
				 * Must be one of the exposed PUBLISH_TO_USER_DEVICES_STATUS enums
				 */
				initialise: function (serverAddress, port, baseURI, serviceProviderId, timeoutCheckMS, locale, region, deviceType, searchURI, useHttps, publishFlagDefaultStatus, forceInitialise) {
					this.init(serverAddress, port, baseURI, serviceProviderId, timeoutCheckMS, locale, region, deviceType, searchURI, useHttps, publishFlagDefaultStatus, forceInitialise);
				},
				...
				/**
				 * Sets the order in which the VOD node (catalogue) data is to be sorted
				 * @method setVODNodeSortOrder
				 * @chainable
				 * @param {Array} sortOrder a list of arrays. Each array consists of a field name and a number which
				 * should be 1 (for ascending order) or -1 (for descending order).
				 */
				setVODNodeSortOrder: function (sortOrder) {
					vodNodeSortOrder = sortOrder;
					return this;
				},
				 ...
				 * @param {Boolean} includeLocale - Include the locale (Default: true)
				 */
				getData: function (caller, successCallback, failureCallback, methodName, filter, sortOrder, fieldList, count, offset, includeLocale) {
					var url;
					apiMethod = methodName;
					url = getServerAddress() + SERVER_URI + '/' + SERVICE_PROVIDER_ID.toUpperCase() + "/" + apiMethod;

					if (!SERVER_ADDRESS || SERVER_ADDRESS === "") {
						failureCallback("No server configured.");
					} else {
						params.filter = filter || DEFAULT_FILTER;

						// TODO: NETUI-4024 - REMOVE THIS CLASS WHEN THESE CHANGES ARE NO LONGER NEEDED
						if (includeLocale !== false && (!filter || !filter.locale)) {
						...
						case EPG_EVENTS_URI:
							...
							break;
						case VOD_TRAILERS_URI:
						case VOD_EDITORIALS_URI:
							params.filter["technical.deviceType"] = "STB";
						...
						case VOD_NODES_URI:
							params.fieldList = fieldList && fieldList.length ? fieldList : vodFieldList;
							params.sortOrder = sortOrder && sortOrder.length ? sortOrder : vodNodeSortOrder;
							break;
				 ...
				 * @param {Number} [count=0] returns only the specified number of records. Defaults to 'everything'
				 * @param {Number} [offset=0] specifies, when used with `count`, what the start offset for the result set should be
			 	 * @param {Boolean} includeLocale Include the locale
				 */
				getVODData: function (caller, successCallback, failureCallback, vodRequestType, filter, sortOrder, fieldList, count, offset, includeLocale) {
					this.getData(caller, successCallback, failureCallback, vodRequestType, filter, sortOrder, fieldList, count, offset, includeLocale);
				},
				...
				fetchSuggestions: function (caller, successCallback, failureCallback, searchTerm, filterQuery) {
					var url = getServerAddress() + SEARCH_SERVER_URI + '/' + SERVICE_PROVIDER_ID.toUpperCase() + SUGGEST_SERVICE_URI + '?q=',
						i,
						searchResult = {},
						MAX_SUGGESTION_ROW = 100; //Maximum 100 suggestions will be fetched
					if (searchTerm) {
						url += encodeURIComponent(searchTerm);
					}
					if (filterQuery && filterQuery.length) {
