...
$N.platform.system.Network = (function () {
	...
	return {
		...
		getIpAddress : function () {
			...
			log("getIpAddress", $N.platform.system.Network.NetworkType.ETHERNET);

			for (i = 0; i < interfacesLength; i++) {
				log("getIpAddress", interfaces[i].name);
				if (interfaces[i].name === $N.platform.system.Network.NetworkType.ETHERNET) {
		...
		renewAddress: function (interfaceType) {
			var network_interface = $N.platform.system.Network.getInterfaceByType(interfaceType);

			if (network_interface !== null) {
				CCOM.IpNetwork.setIpAddress(network_interface.id, "dhcp", "dhcp");
			}
		},
 ...
 */
$N.platform.system.Network.ETHERNET = "eth9";
