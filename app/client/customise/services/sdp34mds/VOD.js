	...
	function (Log, Ratings, ServiceFactory, MetadataService, Promotions, JSON) {
		...
		$N.services.sdp.VOD = (function () {
			...
			function addCommonFilters(deviceTypes, catalogueUid, titleSubstring, filter) {
				...
				filter = addToFilter("isPurchasable", true, filter);
			...
			/**
			 * @method getPurchasedAssetsByProductIdArray
			 * @async
			 ...
			 */
			function getPurchasedAssetsByProductIdArray(productIds, returnCallback, sortArray, returnFailureCallback) {
				log("getPurchasedAssetsByProductIdArray", "Enter");
				var filter,
					successCallback = function (result) {
						if (returnCallback) {
				...
				if (productIds && productIds.length) {
					filter = addToFilter('product.id', {'$in': productIds}, filter);
				}
				...
				log("getPurchasedAssetsByProductIdArray", "Exit");
			}

			/**
			 * @method getSampleAssetsFromNodeArray
			 * @async
			 * @param {Array} nodeIds Array of node IDs that could contain assets
			 * @param {Function} returnCallback
			 * @param {Array} sortArray
			 * @param {Function} returnFailureCallback (optional)
			 */
			function getSampleAssetsFromNodeArray(nodeIds, returnCallback, sortArray, returnFailureCallback) {
				log("getSampleAssetsFromNodeArray", "Enter");
				var filter,
					successCallback = function (result) {
						if (returnCallback) {
							returnCallback(result.editorials);
				...
				if (nodeIds && nodeIds.length) {
					filter = addToFilter('voditem.node', {'$in': nodeIds}, filter);
					filter = addToFilter('product.type', 'subscription', filter);
				}
				filter = addCommonFilters(featureNameList, null, null, filter);
				MetadataService.getVODData(this, successCallback, failureCallback, MetadataService.RequestType.Assets, filter, sortArray, null, 50);
				log("getSampleAssetsFromNodeArray", "Exit");
			}
			 ...
			 * @param {Function} returnFailureCallback (optional)
			 */
			function getAssets(catalogueUID, returnCallback, sortArray, returnFailureCallback) {
				log("getAssets", "Enter");
				var filter,
					successCallback = function (result) {
						if (returnCallback) {
			...
			/**
			 * Retrieves the detailed asset object for the given product id
			 * @method getAssetByProductId
			 * @async
			 * @param {String} id the product id to search for
			 * @param {Function} successCallback The function to run upon successfully returning the asset object
			 * @param {Function} failureCallback the function to run upon failing to retrieve the asset object
			 */
			function getAssetByProductId(id, successCallback, failureCallback) {
				log("getAssetByProductId", "Enter");
				var assetSuccess = function (assetData) {
						if (assetData.error) {
							if (failureCallback) {
								failureCallback();
							...
							log("getAssetByProductId", "Exit");
						}
					},
					filter = addToFilter("editorial.id", id, {});
				filter = addCommonFilters(featureNameList, null, null, filter);
				MetadataService.getVODData(this, assetSuccess, failureCallback, MetadataService.RequestType.Assets, filter);
			}

			/**
			 * Retrieves the detailed asset object for the given filename
			 * @method getAssetByFilename
			 * @async
			 * @param {String} filename the filename associated with the asset to search for.
			 * @param {Function} successCallback The function to run upon successfully returning the asset object
			 * @param {Function} failureCallback the function to run upon failing to retrieve the asset object
			 */
			function getAssetByFilename(filename, successCallback, failureCallback) {
				log("getAssetByFilename", "Enter");
				var assetSuccess = function (assetData) {
						if (assetData.error) {
							if (failureCallback) {
								failureCallback();
							...
							log("getAssetByFilename", "Exit");
						}
					},
					filter = addToFilter("technical.media.AV_EncryptedTS.fileName", filename, {});
				filter = addCommonFilters(featureNameList, null, null, filter);
				MetadataService.getVODData(this, assetSuccess, failureCallback, MetadataService.RequestType.Assets, filter, 1);
			}
			 ...
			 * @method getAssetByFilename
			 * @async
			 * @param {String}[] filenameArray - an array of filenames associated with the asset to search for.
			 * @param {Function} successCallback The function to run upon successfully returning the asset object
			 * @param {Function} failureCallback the function to run upon failing to retrieve the asset object
			 */
			function getAssetsByFilename(filenameArray, successCallback, failureCallback) {
				log("getAssetByFilename", "Enter");
				var assetSuccess = function (assetData) {
						if (assetData.error) {
							if (failureCallback) {
					...
					filter = addToFilter("technical.media.AV_EncryptedTS.fileName", {"$in": filenameArray});
			...
			return {
				...
				getPurchasedAssetsByProductIdArray: getPurchasedAssetsByProductIdArray,
				getSampleAssetsFromNodeArray: getSampleAssetsFromNodeArray,
				getAssets: getAssets,
				...
				getAssetByProductId: getAssetByProductId,
				getAssetByFilename: getAssetByFilename,
				getAssetsByFilename: getAssetsByFilename
			};
		}());
