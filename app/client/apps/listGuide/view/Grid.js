/**
 * Grid is a vertical list of lists.
 * The lists are loaded in as list items
 * via a bridge class, ListLinker.
 *
 * @class $N.gui.Grid
 * @constructor
 * @namespace $N.gui
 ...
 * @requires $N.gui.Util
 * @requires $N.gui.GridRow
 * @param {Object} docRef
 * @param {Object} parent
 */
(function ($N) {
	var Grid = function (docRef, parent) {
		Grid.superConstructor.call(this, docRef);
		this._log = new $N.apps.core.Log("CommonGUI", "Grid");
		this._data.setWrapAround(false);
		this.setOrientation(Grid.consts.ORIENTAION_VERTICAL);
		this._itemPadding = 12;
		this._container = new $N.gui.Group(this._docRef);
		this._items = [];
		this._dataValidationFunction = null;
		this._initialItemCount = 4;
	...
	$N.gui.Util.extend(Grid, $N.gui.AbstractCustomisableList);

	/**
	 * @method getSelectedObject
	 * @return {Object} selectedObject
	 */
	Grid.prototype.getSelectedObject = function () {
	 ...
	 */
	Grid.prototype.setDataValidationFunction = function (testFunction) {
	 ...
	 */
	Grid.prototype.setGridRowItemCallback = function (callback) {
	 ...
	 */
	Grid.prototype.setInitialItemCount = function (initialItemCount) {
	...
	Grid.prototype.setData = function (data) {
		...
		Grid.superClass.setData.call(this, data);
	...
	Grid.prototype.initialise = function () {
		...
		if (!this._itemTemplate) {
			throw ("Grid - initialise: No list item template provided");
		}
	 ...
	 */
	Grid.prototype.doForAllItems = function (runMe) {
		var i;
	 ...
	 */
	Grid.prototype.selectValidatedDataItem = function () {
		var i,
	 ...
	 */
	Grid.prototype.displayData = function (ignoreHighlight) {
		...
		for (i = 0; i < itemsLength; i++) {
			...
			if (itemData) {
				currentItem.setY(currentItemOffset);
				...
				currentItem.show();
				currentItemOffset = currentItemOffset + currentItem.getHeight() + this._itemPadding;
			} else {
				currentItem.hide();
			}
		}
	 ...
	 * @method updateIcons
	 */
	Grid.prototype.updateIcons = function () {
	 ...
	 */
	Grid.prototype.selectPrevious = function () {
	 ...
	 * @method selectNext
	 */
	Grid.prototype.selectNext = function () {
	 ...
	 */
	Grid.prototype.selectRowAtIndex = function (index, ignoreHighlight) {
	 ...
	 */
	Grid.prototype.keyHandler = function (key) {
		...
		if (!handled) {
			...
			/* This compontent is hard coded to horizontal navigation so up/down
			is assumed to not be the next/previous keys */
			case keys.KEY_UP:
			case keys.KEY_DOWN:
				Grid._inKeyRepeat = true;
				break;
	...
	/**
	 * @method getFocusState
	 * @return {String}
	 */
	Grid.prototype.hasFocus = function () {
		return this._focusState;
	};

	Grid._inKeyRepeat = false;
	Grid.consts = $N.gui.AbstractCustomisableList.consts;
