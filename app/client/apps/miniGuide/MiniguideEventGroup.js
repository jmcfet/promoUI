...
function MiniguideEventGroup(docRef, parent) {
	...
	this._log = new $N.apps.core.Log('MINIGUIDE', 'MiniguideEventGroup');
...
MiniguideEventGroup.prototype.setAudioLanguageTitle = function () {
	...
	if (audio) {
		...
		if ((availableAudioTracksLength === 1) || isInvalidAudioTrack) {
...
MiniguideEventGroup.prototype.activateSubtitle = function () {
	...
	if (currentSubtitleText === this.dictionary.on) {
		$N.app.fullScreenPlayer.tracks.activateSubtitleTrackByLanguage($N.app.TracksUtil.getLanguageforSubtitleTrack());
