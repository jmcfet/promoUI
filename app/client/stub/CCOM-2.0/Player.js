...
CCOM.PlayerManager = (function () {
	...
	defaultPlayer.instance = {
		...
		PLAY_CONTROL_CMD_INVALID: 2,
		PLAY_CONTROL_CMD_POSITION: 3,
		...
		videoDetails: {},
		bufferingLevel: 60,
		destUri: "http://test.destUri",
		videoBlankStatus: false,
		...
		startStreams: function (context) {
			...
			for (contextCount = 0; contextCount < context.length; contextCount++) {
				...
				if (stream) {
					...
					this.activeStreams.push({id: stream.id, type: stream.type});
				}
			}
		},

		stopStreams: function (context) {
			var newActiveStreams = [];
			var contextCount;
			var i;
			for (contextCount = 0; contextCount < context.length; contextCount++) {
				for (i = 0; i < this.activeStreams.length; i++) {
					...
					if (sourceUri != this.sourceUri) {
						this.sourceUri = sourceUri;
							...
							if( video_obj.src.indexOf("1.ogv") > 0 )  {
									video_obj.src = "stub/CCOM-2.0/video/2.ogv";
							} else {
									video_obj.src = "stub/CCOM-2.0/video/1.ogv";
							}
						}
		},
		...
		getStreamControl: function () {
			return {streamCommandControlData:{}};
		},
		...
		setVideoLayerDetails: function (configObject) {
			videoLayerProperties = configObject;
		}
	};
